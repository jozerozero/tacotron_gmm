...
hparams = tf.contrib.training.HParams(
	...
	tacotron_num_gpus = NGPU_tac, #Determines the number of gpus in use for Tacotron training.
	wavenet_gpu_start_idx = 4, #idx of the first GPU to be used for WaveNet training. (WIP)
	wavenet_num_gpus = NGPU_wav, #Determines the number of gpus in use for WaveNet training. (WIP)
	...
	tacotron_batch_size = 64 * NGPU_tac, #number of training samples on each training steps
	...
	tacotron_test_size = 0.064, #% of data to keep as test data, if None, tacotron_test_batches must be not None. (5% is enough to have a good idea about overfit)
	...
	#train/test split ratios, mini-batches sizes
	wavenet_batch_size = 8 * NGPU_wav, #batch size used to train wavenet.
