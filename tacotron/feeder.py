...
class Feeder:
    ...
    def __init__(self, coordinator, metadata_filename, hparams):
        ...
        self._mel_dir = os.path.join(
            os.path.dirname(metadata_filename), 'mels')
        self._linear_dir = os.path.join(
            os.path.dirname(metadata_filename), 'linear')
        ...
        len_test_indices = self._round_down(
            len(test_indices), hparams.tacotron_batch_size)
        ...
        with tf.device('/cpu:0'):
            ...
            self._placeholders = [
                ...
                tf.placeholder(tf.float32, shape=(
                    None, None, hparams.num_mels), name='mel_targets'),
                tf.placeholder(tf.float32, shape=(
                    None, None), name='token_targets'),
                tf.placeholder(tf.float32, shape=(
                    None, None, hparams.num_freq), name='linear_targets'),
            ...
            eval_queue = tf.FIFOQueue(
                1, [tf.int32, tf.int32, tf.float32, tf.float32, tf.float32, tf.int32, tf.int32], name='eval_queue')
    ...
    def start_threads(self, session):
        ...
        thread = threading.Thread(
            name='background', target=self._enqueue_next_test_group)
        thread.daemon = True  # Thread will close when parent quits
        thread.start()
    ...
    def make_test_batches(self):
        ...
        examples = [self._get_test_groups()
                    for i in range(len(self._test_meta))]
    ...
    def show_one_in_64_train_feeddict(self):
        start = time.time()

        # Read a group of examples
        n = self._hparams.tacotron_batch_size
        r = self._hparams.outputs_per_step
        if self._train_offset != 0:
            return 'should only be  before start_threads'
